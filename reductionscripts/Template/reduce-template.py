        ...
        if iter == 1:
            ...
            if iter == 2:
                ...
                if np.any(scanreduced==False):
                    mymodel = createSourceModel(coadded, highcut=5.5, lowcut=2.5, sm=0., mtype='snr', clip=1.0)
                else:
                    mymodel = None
            ...
            if iter == 3:
                subtract = True
                if np.any(scanreduced==False):
                    mymodel = createSourceModel(coadded, highcut=5.5, lowcut=2.5, sm=0., mtype='flux', clip=1.0)
                else:
                    mymodel = None
