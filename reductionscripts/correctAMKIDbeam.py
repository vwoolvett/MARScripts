...
with warnings.catch_warnings():
    ...
    for iter in range(1, niters+1):
        ...
        if iter==1:
            ...
            correctionsummary += '\nSmoothing was:                              %1.3f "'%(smoothby_deg*3600.)
            correctionsummary += '\nUnsmoothed beam:                            %.3f "'%(UNCORRECTED_NATIVE_FWHM*3600.)
            correctionsummary += '\nAMKID median beam:                          %.3f "'%(CORRECT_NATIVE_FWHM*3600.) + ' (%s)'%from_where
            correctionsummary += '\nSky map (image) was rescaled by:            %.3fx'%(correct_scale/uncorrected_scale)
            correctionsummary += '\nVariance map (/Weight) was rescaled by:     %.3fx'%(correct_scale/uncorrected_scale)
